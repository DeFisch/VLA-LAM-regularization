...
class CollatorForLatentAction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        initial_pixel_values = [instance["initial_pixel_values"] for instance in instances]
        initial_pixel_values = [torch.nn.functional.interpolate(ipv.unsqueeze(0), size=(256, 256), mode='bilinear', align_corners=False).squeeze(0) for ipv in initial_pixel_values]
        initial_pixel_values = torch.stack(initial_pixel_values)
        
        target_pixel_values = [instance["target_pixel_values"] for instance in instances]
        target_pixel_values = [torch.nn.functional.interpolate(tpv.unsqueeze(0), size=(256, 256), mode='bilinear', align_corners=False).squeeze(0) for tpv in target_pixel_values]
        target_pixel_values = torch.stack(target_pixel_values)
