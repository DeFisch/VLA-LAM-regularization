...
class FSDPStrategy(TrainingStrategy):
    def __init__(
        ...
        if self.fsdp_sharding_strategy != ShardingStrategy.NO_SHARD:
            self.fsdp_save_policy = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
        else:
            self.fsdp_save_policy = FullStateDictConfig(offload_to_cpu=False, rank0_only=True)
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        if self.fsdp_sharding_strategy == ShardingStrategy.NO_SHARD:
            vlm_fsdp_wrapping_policy = None  # No wrapping needed if no sharding
            limit_all_gathers = False  # No need to limit all-gathers if no sharding
        ...
        if dist.is_initialized():
            ...
            print("Warning: Distributed not initialized, training on single GPU without FSDP!")
            self.vlm = self.vlm.to(self.device_id)

        # Gradient Checkpoint Setup
        if self.enable_gradient_checkpointing and not (self.fsdp_sharding_strategy == ShardingStrategy.NO_SHARD):
        ...
        # debug df
        if self.fsdp_sharding_strategy != ShardingStrategy.NO_SHARD: # only barrier if sharding
            dist.barrier()
        ...
        # Finalize Setup =>> Log!
        overwatch.info(
            f"{self.fsdp_sharding_strategy} Strategy =>> Finalized Training Setup:\n"
    ...
    def clip_grad_norm(self) -> None:
        ...
        # debug df
        if self.fsdp_sharding_strategy == ShardingStrategy.NO_SHARD:
            torch.nn.utils.clip_grad_norm_(self.vlm.parameters(), self.max_grad_norm)
        else:
