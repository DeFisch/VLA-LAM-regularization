...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        # Stack LAM latent indices (if present)
        if "lam_latent_indices" in instances[0] and instances[0]["lam_latent_indices"] is not None:
            lam_latent_indices = [instance["lam_latent_indices"] for instance in instances]
            lam_latent_indices = torch.stack(lam_latent_indices)
        else:
            lam_latent_indices = None
        ...
        if lam_latent_indices is not None:
            output["lam_latent_indices"] = lam_latent_indices
        return output
