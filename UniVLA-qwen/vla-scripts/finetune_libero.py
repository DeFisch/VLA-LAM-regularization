...
class FinetuneConfig:
    ...
    lam_path: str = "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/checkpoints/univla-latent-action-model/lam-stage-2.ckpt"
    # Directory Paths
    data_root_dir: Path = Path("/fs/scratch/PAS2099/danielf/geometry_grounded_latents/LIBERO/data/modified_libero_rlds")      # Path to Open-X dataset directory
    dataset_name: str = "libero_goal_no_noops"                   # Name of fine-tuning dataset (e.g., `droid_wipe`)
    run_root_dir: Path = Path("qwen-finetuned-goal-continue")                               # Path to directory to store logs & checkpoints
    adapter_tmp_dir: Path = Path("qwen-finetuned-goal-continue")                     # Temporary directory for LoRA weights before fusing

    # Fine-tuning Parameters
    batch_size: int = 12                                             # Fine-tuning batch size
    max_steps: int = 20000                                          # Max number of fine-tuning steps
    save_steps: int = 1000                                         # Interval for checkpoint saving
    learning_rate: float = 3.5e-5                                   # Fine-tuning learning rate
    ...
    resume_vla_ckpt: Optional[Path] = "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/vla-scripts/qwen-finetuned-goal/pretrained-univla-7b+libero_goal_no_noops+b12+lr-3.5e-05--image_aug=w-LowLevelDecoder-ws-12/vla-full-8000.pt"
    resume_action_decoder_ckpt: Optional[Path] = "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/vla-scripts/qwen-finetuned-goal/pretrained-univla-7b+libero_goal_no_noops+b12+lr-3.5e-05--image_aug=w-LowLevelDecoder-ws-12/action_decoder-8000.pt"
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # )
    vla = load('/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/checkpoints/prism-qwen25-extra-dinosiglip-224px-0_5b', load_for_training=not cfg.freeze_vla)
    trained_model = '/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/vla-scripts/univla-1b/2809516--image_aug-Latent-Action-Pretraining/checkpoints/step-070000-epoch-09-loss=0.3847.pt'
    raw = torch.load(trained_model, map_location="cpu")['model']  # or "cuda" if you prefer
    converted = flatten_prismatic_ckpt(raw, to_cpu=True, cast_dtype=torch.bfloat16, fix_layerscale=False)

    missing_keys, unexpected_keys = vla.load_state_dict(converted, strict=False)
