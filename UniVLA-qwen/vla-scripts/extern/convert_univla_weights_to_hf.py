...
class HFConvertConfig:
    ...
    openvla_model_path_or_id: Union[str, Path] = (                      # Path to Pretrained VLA (on disk or HF Hub)
        "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/vla-scripts/univla-1b-final"
    )
    ckpt_name: str = '/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/vla-scripts/univla-1b-final/checkpoints/step-072270-epoch-10-loss=0.4059.pt'                   # The specific checkpoint to be converted (modify accordingly)
    output_hf_model_local_path: Path = Path(                            # Path to Local Path to save HF model
        "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/vla-scripts/univla-1b-final/converted"
    )
...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    with open(config_json, "r") as f:
        ...
        # 1b model
        model_config_path = "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/checkpoints/prism-qwen25-extra-dinosiglip-224px-0_5b/config.json"
        prismatic_config = json.load(open(model_config_path, "r"))
        prismatic_config = prismatic_config['model']
    ...
    #     hf_config.llm_backbone_id, model_max_length=hf_config.llm_max_length, token=cfg.hf_token, padding_side="right"
    # )
    from prismatic.models.materialize import get_llm_backbone_and_tokenizer
    _, tokenizer = get_llm_backbone_and_tokenizer(
        prismatic_config["llm_backbone_id"],
        llm_max_length=prismatic_config.get("llm_max_length", 2048),
    ...
    special_tokens_dict = {'additional_special_tokens': [f"<|extra_{i}|>" for i in range(cfg.codebook_size)]}
    ...
    # model_state_dict = flatten_prismatic_ckpt(model_state_dict, to_cpu=True, cast_dtype=torch.bfloat16, fix_layerscale=False)
