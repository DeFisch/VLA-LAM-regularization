...
def _bootstrap_dist_env():
    """
    Initialize torch.distributed via env:// if RANK/WORLD_SIZE exist.
    Sets CUDA device from LOCAL_RANK. Works for single-GPU too.
    """
    # Respect torchrun / SLURM exporters
    have_env = ("RANK" in os.environ) and ("WORLD_SIZE" in os.environ)
    print(f"Rank exists: {'RANK' in os.environ}, World Size exists: {'WORLD_SIZE' in os.environ}")
    print(f"dist init: {dist.is_initialized()}")
    if have_env and not dist.is_initialized():
        dist.init_process_group(
            backend="nccl",
            init_method="env://",
            timeout=timedelta(minutes=30),
        )
        print("torch.distributed initialized via env://")
    else:
        print("torch.distributed not initialized; running in single-process mode.")
...
class TrainConfig:
    ...
    pretrain_vlm: str = '/fs/scratch/PAS2099/danielf/geometry_grounded_latents/UniVLA/checkpoints/prism-qwen25-extra-dinosiglip-224px-0_5b'
    ...
    data_root_dir: Path = Path(                                     # Path to Open-X dataset directory
        "/fs/scratch/PAS2099/danielf/geometry_grounded_latents/data"
    )
    run_root_dir: Path = Path("temp/univla-1b")                               # Path to directory to store logs & checkpoints
    ...
    hf_token: Union[str, Path] = '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'                
    ...
    codebook_size: int = 16                                       # Size of the latent action codebook
    batch_per_device: int = 24                                        # Batch size per device
    ...
    def __post_init__(self) -> None:
        ...
        # change for 7b
        self.train_strategy = "fsdp-no-shard"
...
def train(cfg: TrainConfig) -> None:
    print(f"Distributed initialized: {using_dist}")
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        overwatch.info(f"Dist OK: world={world} rank={rank} local_rank={local}")
    else:
        overwatch.info("Running single-process (WORLD_SIZE=1)")

    # Note => Under `torchrun` initializing `overwatch` will automatically set up `torch.distributed`
    # debug df
    if dist.is_initialized():
        torch.cuda.set_device(device_id := int(os.environ.get("LOCAL_RANK", 0)))    # device_id = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    else:
    ...
    overwatch.info(f"Initializing Train Strategy `{cfg.train_strategy}`, with {overwatch.world_size()} GPUs")
    ...
    train_strategy = get_train_strategy(
        ...
        # debug df
        global_batch_size=cfg.batch_per_device * overwatch.world_size(),
        per_device_batch_size=cfg.batch_per_device,
