...
def load(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        if any('.safetensors' in f.name for f in run_dir.glob('*')):
            overwatch.info(f"Detected HuggingFace safetensors format in `{run_dir}`")
            ...
            model = AutoModelForVision2Seq.from_pretrained(run_dir, torch_dtype=torch.bfloat16, device_map="cpu", 
            ...
            print("Loaded HuggingFace format model")
